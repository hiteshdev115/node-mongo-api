    ...
    if(isLoading === true){
      ...
      for(let i = 0; i< allSeo.length; i++){
        ...
        dataTableValue.push({
          ...
          action:<div><a href={"./editseo/"+allSeo[i]._id} className="trash"><MDBIcon icon="pen-alt" /></a> | 
          <a onClick={() => this.deleteSeo(allSeo[i]._id)} className="trash"><MDBIcon far icon="trash-alt" /></a></div>
        });
      }
    }
